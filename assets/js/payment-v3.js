...
function copyOrderRefClipboard(){
    let order_ref_no = $('#success_order_ref_no').val();
    $("#alert_message_wrapper").removeAttr('hidden', 'hidden').text(order_ref_no+' copied!');
    $('.alert-message').text($(this).data('text')).fadeIn(400).delay(3000).fadeOut(400);
    return order_ref_no; 
}
...
function getTransactionData(){
    ...
    .done(function(data) {
        retrievePaymentSource(data.payment_src_id, data.token, data.reference_no);
    })
}

/* Retrive payment source details */ 
function retrievePaymentSource(payment_src_id, token, reference_no){
    var data = null;
    ...
    xhr.addEventListener("readystatechange", function () {
      if (this.readyState === this.DONE) {
        ...
        if (source.data.attributes.status == 'chargeable') { /* check if customers already paid*/
            authorizePaymentResource(source.data.attributes.amount, payment_src_id, token, reference_no);
        }
           ...
           $("#payment_source_btn").attr('onclick', 'openPaymentPage(\''+source.data.attributes.redirect.checkout_url+'\',\''+source.data.id+'\',\''+token+'\',\''+reference_no+'\')')
        }
...
/* Open payment page*/ 
function openPaymentPage(url, id, token, reference_no){
    window.open(""+url+"", "_self"); /* payment page */

    checkPaymentStatus(id, token, reference_no);
}

/* Check the payment status */ 
function checkPaymentStatus(id, token, reference_no){
    var data = null;
    ...
    xhr.addEventListener("readystatechange", function () {
        if (this.readyState === this.DONE) {
            var source = JSON.parse(this.response);
            if (source.data.attributes.status == 'chargeable') { /* check if customers already paid*/
                authorizePaymentResource(source.data.attributes.amount, id, token, reference_no)
            }
            else{
                setTimeout(function(){ checkPaymentStatus(id, token, reference_no) }, 3000)
            }
        }
    });
...
function authorizePaymentResource(amount, id, token, reference_no){
    source_id = id;
    store = 'KenKarlo Digital';
    message = 'Order complete! Order ref. #'+reference_no;
...
function transactionStatus(data){
    ...
    string+='<div class="margin-bottom-40" id="form_transaction_wrapper">'
        +'<div class=" margin-top-10">'
            ...
            +'<div class="row invoice-header-wrapper margin-bottom-20" >'
                +'<div class="col-xs-9 invoice-header" ><h4 class="font-300">Transaction Details</h4></div>'
                +'<div class="col-xs-3 text-right print-wrapper"><a class="btn btn-default" href="'+base_url+'v3/order/'+data.reference_no+'/invoice?utm_souce=order_page" onclick="printInvoice()" ><i class="fa fa-receipt"></i> Want a receipt?</a></div>'
            +'</div>'
            ...
            +'<div class="row  margin-top-10">'
                +'<div class="col-md-6 col-5"><span class="font-600 label-payment">Order Ref. No.</span></div>'
                    +'<input type="hidden" id="success_order_ref_no" value="'+data.reference_no+'">'   
                    +'<div class="col-md-6 col-6"><span class="font-400 desc-payment cursor-pointer" id="copy_order_ref">'+data.reference_no+'</span>  <span id="copy_order_ref" class="font-sm cursor-pointer"><i class="fa fa-copy"></i></span></div>'
                +'</div>'
            +'</div>'
        ...
        // +'<div class="text-left margin-bottom-20" style="margin-top: -20px;">'
        //     +'<div class=""><span class="font-400"><a target="_blank" class="font-black" href="'+base_url+'v3/order/'+data.reference_no+'/invoice?utm_souce=order_page" target="_blank"><i class="fa fa-receipt"></i> Get Invoice</a></div>'
        // +'</div>'
